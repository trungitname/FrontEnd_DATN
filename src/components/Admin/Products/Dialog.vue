  ...
  <v-dialog max-width="500px" v-model="showDialog">
    <v-card>
      <v-card-title style="font-weight: bold;">
        <h4>{{ isEdit ? 'Chỉnh sửa sản phẩm' : 'Tạo mới sản phẩm' }}</h4>
      </v-card-title>
      <v-container style="background-color: rgb(247, 247, 247);">
        <v-row>
          ...
          <v-col cols="12">
            <v-combobox v-model="newProduct.size" :items="items" label="Chọn sizes" chips multiple style="background-color: white;" dense outlined></v-combobox>
          </v-col>
...
export default {
  ...
  methods: {
    ...
    async addProduct() {
      ...
      formData.append('color', JSON.stringify(this.newProduct.color)); // Chuyển đổi thành chuỗi JSON
      formData.append('size', JSON.stringify(this.newProduct.size)); // Chuyển đổi thành chuỗi JSON
      ...
      try {
        ...
        this.closeDialog();
        this.$emit('productAdded'); // Thông báo cho parent component để cập nhật danh sách sản phẩm
      } catch (error) {
